...
URDF = 'HexaMotion-SDK/robots/HexaArm_Mini_Nema_Assem.SLDASM5/urdf/HexaArm_Mini_Nema_Assem.SLDASM5.urdf'
...
candidates = []
for a2 in (15, 25, 35, 45, -15, -25, -35, -45):
    for a3 in (-60, -50, -35, -20, 20, 35, 50, 60):
        for a5 in (-70, -60, -40, -20, 20, 40, 60, 70):
...
for q_ref in candidates:
    ...
    up = p[UP]
    if horiz < 200 or horiz > 330:   # comfortably inside reach for a ~450mm arm
        continue
    # Boss v4: the pattern sits HIGHER and FURTHER OUT from the base than the first demo cut.
    # Boss v5 (2026-07-06): pushed further out again (target 255 -> 275 mm from the base axis).
    if up < 210 or up > 330:
        continue
    scored.append((abs(horiz-275) + abs(up-275), q_ref, p))
...
CORNERS_FULL = [(-150, -105), (-90, 105), (-30, -105), (30, 105), (90, -105), (150, 105)]
CORNERS = CORNERS_FULL  # rescaled by the search loop below
PASS_UP = {'PTP': 75.0, 'LIN': 25.0, 'CIRC': -25.0, 'SPLINE': -75.0}
...
doc = {'formatVersion': 2, 'program': {'name': 'MotionTypesDemo', 'steps': steps}}
import os
out = 'HexaMotion/programs/motion_types_demo.json'
