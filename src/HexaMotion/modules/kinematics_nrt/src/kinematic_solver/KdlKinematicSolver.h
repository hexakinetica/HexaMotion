...
private:
    ...
    // Returns a typed error instead of throwing on a size mismatch (mandate: no exceptions). The
    // mismatch is not expected (solveIK sizes the result array to the DOF) but is guarded so a KDL
    // contract violation surfaces as an IK failure rather than undefined behaviour or a throw.
    [[nodiscard]] Result<AxisSet, IKError> fromKdlJntArray(const KDL::JntArray& kdl_jnts) const;
