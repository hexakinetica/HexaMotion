...
- `KinematicModel` loads URDF and keeps joint/segment metadata.
- `FrameTransformer` converts tool/base frames using stored transforms.
- `KinematicSolver` is the abstract base for IK/FK implementations (`KdlKinematicSolver`).

## 5. Test Recommendations

**Unit Tests**
- FK/IK round-trip for a known URDF configuration.
- `FrameTransformer` handles missing frames with explicit error codes.

**Integration Tests**
- Planner → Kinematics conversion of waypoint frames.