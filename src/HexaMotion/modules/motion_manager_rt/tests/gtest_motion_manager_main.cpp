...
// A minimal HAL stub to force read/write errors for testing
class FailingHardwareManager : public HardwareManager {
public:
    FailingHardwareManager(const InterfaceConfig& config, const RobotLimits& limits)
        : HardwareManager(config, limits) {}

    Result<void, ErrorCode> write(const HardwareCommand& cmd) override {
        (void)cmd;
...
TEST_F(MotionManagerTest, ExecutesValidMotion) {
    ...
    // The Idle transition happens in the SAME cycle that streams the LAST point, and SimDriver
    // models a 1-cycle drive latency, so that cycle's feedback still reports the previous
    // setpoint. An immediate drain therefore deterministically reads one point short (9/18);
    // the correct production observation is the NEXT (HOLD) cycle. Poll bounded until the
    // actual settles at the final target.
    TrajectoryPoint fb;
    bool settled = false;
    const auto deadline = std::chrono::steady_clock::now() + 1000ms;
    while (std::chrono::steady_clock::now() < deadline) {
        TrajectoryPoint p;
        while (mm->dequeueFeedback(p)) {
            fb = p;
        }
        if (std::abs(fb.feedback.joint_actual.GetAt(0).value().get().position.value() - 10.0) < 0.01 &&
            std::abs(fb.feedback.joint_actual.GetAt(1).value().get().position.value() - 20.0) < 0.01) {
            settled = true;
            break;
        }
        std::this_thread::sleep_for(10ms);
    }
    ASSERT_TRUE(settled) << "Final actual did not settle at the commanded target within 1 s";
...
// Test HAL error propagation (REQ-MM-09)
TEST_F(MotionManagerTest, HalErrorPropagation) {
    auto failing_hw = std::make_shared<FailingHardwareManager>(config, limits);
    ASSERT_TRUE(failing_hw->init().isSuccess());
    mm = std::make_unique<MotionManager>(failing_hw, 50, limits, 5.0_deg);

    // Start should fail because read() fails
    EXPECT_FALSE(mm->start());
    EXPECT_EQ(mm->getCurrentState(), RTState::Error);
}

// --- RtPointRing (TrajectoryPoint cleanup Batch D): the RT loop's zero-allocation buffer --------

namespace {
// Distinguishable point: the sequence index identifies FIFO order through the ring.
TrajectoryPoint ringPoint(uint32_t seq) {
    TrajectoryPoint p;
    p.header.sequence_index = seq;
    return p;
}
} // namespace

TEST(RtPointRing, FifoOrderSurvivesWrapAround) {
    // Push/pop far past kCapacity so the head wraps many times: order must stay strictly FIFO.
    // This mirrors the production pattern exactly: fill to the refill threshold (25), then one
    // pop + one push per RT cycle.
    RtPointRing ring;
    uint32_t next_in = 0;
    uint32_t next_out = 0;
    for (int i = 0; i < 25; ++i) {
        ASSERT_TRUE(ring.push_back(ringPoint(next_in++)));
    }
    for (int cycle = 0; cycle < 1000; ++cycle) {
        ASSERT_FALSE(ring.empty());
        EXPECT_EQ(ring.front().header.sequence_index, next_out++);
        ring.pop_front();
        ASSERT_TRUE(ring.push_back(ringPoint(next_in++)));
        EXPECT_EQ(ring.size(), 25u);
    }
}

TEST(RtPointRing, RefusesPushWhenFullAndKeepsContents) {
    RtPointRing ring;
    for (std::size_t i = 0; i < RtPointRing::kCapacity; ++i) {
        ASSERT_TRUE(ring.push_back(ringPoint(static_cast<uint32_t>(i))));
    }
    EXPECT_EQ(ring.size(), RtPointRing::kCapacity);
    // A full ring refuses (fail-visible contract) and must not corrupt the buffered points.
    EXPECT_FALSE(ring.push_back(ringPoint(999)));
    EXPECT_EQ(ring.size(), RtPointRing::kCapacity);
    EXPECT_EQ(ring.front().header.sequence_index, 0u);
}

TEST(RtPointRing, ClearEmptiesAndReuses) {
    RtPointRing ring;
    ASSERT_TRUE(ring.push_back(ringPoint(1)));
    ASSERT_TRUE(ring.push_back(ringPoint(2)));
    ring.clear();
    EXPECT_TRUE(ring.empty());
    EXPECT_EQ(ring.size(), 0u);
    ASSERT_TRUE(ring.push_back(ringPoint(7)));
    EXPECT_EQ(ring.front().header.sequence_index, 7u);
}

TEST(RtPointRing, CapacityCoversRefillThreshold) {
    // The refill loop tops up to RT_BUFFER_REFILL_THRESHOLD (25); the ring must always have room
    // for it (the push_back refusal is a diagnostic guard, not a reachable production path).
    EXPECT_GT(RtPointRing::kCapacity, static_cast<std::size_t>(25));
}
